<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.kebiaotupian" label="课表图片" prop="kebiaotupian">
          <file-upload
          tip="点击上传课表图片"
          action="file/upload"
          :limit="3"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.kebiaotupian" label="课表图片" prop="kebiaotupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.kebiaotupian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="课表详情" prop="kebiaoxiangqing">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.kebiaoxiangqing" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.kebiaoxiangqing" label="课表详情" prop="kebiaoxiangqing">
                    <span v-html="ruleForm.kebiaoxiangqing"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.faburiqi = true;
          ...
          if(o=='kebiaoxiangqing'){
            this.ruleForm.kebiaoxiangqing = obj[o];
	    this.ro.kebiaoxiangqing = true;
            continue;
          }
          if(o=='jiaoshigonghao'){
	...
	this.ruleForm.kebiaoxiangqing = this.ruleForm.kebiaoxiangqing.replace(reg,'../../../ssmt1g73/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jiaoshikebiao/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jiaoshikebiaoCrossAddOrUpdateFlag = false;
						 this.parent.search();
